...
def threads_list(request):
    ...
    return render(request, 'posts/threads_list.html', {'threads': threads})
...
def create_thread(request):
    if request.method == 'POST':
        ...
        form = ThreadForm()
    return render(request, 'posts/create_thread.html', {'form': form})

# Детали Thread
def thread_detail(request, id):
    thread = get_object_or_404(Thread, id=id)
    posts = Post.objects.filter(thread=thread)
    return render(request, 'posts/thread_detail.html', {'thread': thread, 'posts': posts})
...
def create_post(request, thread_id):
    ...
    if request.method == 'POST':
        ...
        form = PostForm()
    return render(request, 'posts/create_post.html', {'form': form, 'thread': thread})
